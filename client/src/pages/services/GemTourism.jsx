    ...
    <>
      ...
      <section className="py-20 lg:py-28 bg-white relative overflow-hidden">
        ...
        <div className="relative container-x">
          ...
          <Reveal delay={0.05}>
            <h2 className="font-display text-4xl md:text-6xl lg:text-7xl font-medium leading-[1.02] tracking-tight text-ink max-w-5xl mb-8">
      ...
      <section className="py-20 lg:py-24 bg-white">
        <div className="container-x">
          <Reveal><div className="max-w-3xl mb-14"><span className="eyebrow">Everything Taken Care Of</span><h2 className="section-title mb-5">What{'\u2019'}s <em>included</em> in every tour.</h2><p className="text-ink-soft text-lg">Register with us, fly to Sri Lanka, and we handle the rest.</p></div></Reveal>
            ...
            {included.map(({ icon: Icon, label, detail }, i) => (
              <Reveal key={label} delay={i * 0.06}>
                <div className="h-full bg-cream/60 rounded-2xl p-6 border border-line hover:shadow-card hover:-translate-y-1 transition-all duration-500 group">
                  <div className="w-12 h-12 rounded-xl bg-sapphire-light text-sapphire grid place-items-center mb-4 group-hover:bg-sapphire group-hover:text-white transition-colors duration-500"><Icon size={22} strokeWidth={1.8} /></div>
                  <h3 className="font-semibold text-ink mb-2 text-[0.95rem]">{label}</h3>
      ...
      <section className="py-20 lg:py-24 bg-cream">
        <div className="container-x">
          <Reveal><div className="max-w-3xl mb-12"><span className="eyebrow">Your Comfort, Our Priority</span><h2 className="section-title mb-5">Transport & <em>Accommodation</em></h2><p className="text-ink-soft text-lg">Airport pickup, comfortable domestic travel, and free lodging at our own property.</p></div></Reveal>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <Reveal delay={0.05}><div><ImagePlaceholder label="Airport pickup" filename="transport-1.jpg" aspect="4/3" /><h3 className="font-display text-xl font-semibold text-ink mt-4 mb-1">Airport Pickup</h3><p className="text-sm text-muted">We meet you at Bandaranaike International and drive you straight to our property.</p></div></Reveal>
            <Reveal delay={0.1}><div><ImagePlaceholder label="Domestic transport" filename="transport-2.jpg" aspect="4/3" /><h3 className="font-display text-xl font-semibold text-ink mt-4 mb-1">Domestic Travel</h3><p className="text-sm text-muted">All travel between mines, markets and your accommodation is fully covered.</p></div></Reveal>
            <Reveal delay={0.15}><div><ImagePlaceholder label="Free accommodation" filename="transport-3.jpg" aspect="4/3" /><h3 className="font-display text-xl font-semibold text-ink mt-4 mb-1">Free Accommodation</h3><p className="text-sm text-muted">Stay at our own property with complimentary home-cooked meals. Outside dining is at your own expense.</p></div></Reveal>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-24 bg-white">
        <div className="container-x">
          <Reveal><div className="max-w-3xl mb-14"><span className="eyebrow">What You{'\u2019'}ll Experience</span><h2 className="section-title mb-5">Immersive Tour Activities & <em>Experiences</em></h2><p className="text-ink-soft text-lg">From the raw earth of the mine to a polished gem in your hand {'\u2014'} and everything in between.</p></div></Reveal>
            ...
            {experiences.map((exp, i) => (
              <Reveal key={exp.filename} delay={i * 0.08}>
                <article className="group bg-cream rounded-2xl overflow-hidden border border-line hover:shadow-card transition-all duration-500">
                  ...
                  <div className="p-7"><h3 className="font-display text-2xl font-semibold text-ink mb-3 group-hover:text-sapphire transition-colors">{exp.title}</h3><p className="text-ink-soft leading-relaxed">{exp.text}</p></div>
                </article>
              </Reveal>
            ))}
          </div>
      ...
      <section className="py-20 lg:py-24 bg-ink text-white relative overflow-hidden">
        ...
        <div className="relative container-x">
          <Reveal><div className="max-w-3xl mb-14"><div className="text-xs tracking-[0.3em] uppercase text-gold font-bold mb-4">Transparent Pricing</div><h2 className="font-display text-4xl lg:text-5xl xl:text-6xl font-medium leading-tight mb-5">No hidden fees.<br /><span className="text-gold italic">Just 10% of your purchase.</span></h2><p className="text-white/70 text-lg">Our tour service fee is simple: 10% of the total value of gems you purchase. Transport, accommodation, meals and guide services are all included. Gem certification is included too. Island-wide sightseeing is quoted separately upfront.</p></div></Reveal>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {pricingPoints.map(({ icon: Icon, title, text }, i) => (
              <Reveal key={title} delay={i * 0.08}>
                <div className="h-full bg-white/5 rounded-2xl p-6 border border-white/10 hover:border-gold/30 transition-colors">
                  ...
                  <h3 className="font-semibold text-white mb-2 text-[0.95rem]">{title}</h3>
              ...
              <div className="bg-cream rounded-3xl p-8 lg:p-10 border border-line">
                ...
                <ul className="space-y-4">
                  {['Register before coming to Sri Lanka','Airport pickup + domestic transport \u2014 free','Free accommodation + meals at our place','Guided visits to mines, workshops & markets','Purchasing at Beruwala & Ratnapura arranged','Only 10% of your gem purchase value','Gem certification arranged','Island-wide Sri Lanka tours available (extra)'].map((item) => (
                    <li key={item} className="flex items-start gap-3 text-ink-soft"><CheckCircle2 size={18} className="text-sapphire mt-0.5 shrink-0" /><span className="text-[0.95rem] leading-snug">{item}</span></li>
                  ))}
                </ul>
              </div>
            </Reveal>
      ...
      <section className="py-16 lg:py-20 bg-ink text-white">
        <div className="container-x">
          <div className="grid lg:grid-cols-[1.4fr_1fr] gap-10 items-center">
            <Reveal><div><div className="text-xs tracking-[0.3em] uppercase text-gold font-bold mb-4">Ready to Experience Ceylon?</div><h2 className="font-display text-4xl lg:text-5xl font-medium leading-tight mb-4">Register today.<br /><span className="text-gold italic">We{'\u2019'}ll take it from there.</span></h2><p className="text-white/70 max-w-xl">Send us a message or chat on WhatsApp. Our tour coordinator will be in touch within 24 hours.</p></div></Reveal>
            <Reveal delay={0.15}><div className="flex flex-wrap gap-4 lg:justify-end">
              <Link to="/contact" className="inline-flex items-center gap-3 px-7 py-4 rounded-full font-semibold text-[0.92rem] bg-white text-ink hover:bg-cream transition-all">Register Now <ArrowRight size={16} strokeWidth={2.4} /></Link>
              <a href={whatsappHref("Hi! I'd like to register for a gem tour in Sri Lanka.")} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 px-7 py-4 rounded-full font-semibold text-[0.92rem] border border-white/25 text-white hover:bg-white/10 transition-all">Chat on WhatsApp</a>
            </div></Reveal>
          </div>
        </div>
      </section>
...
function StepCard({ step, highlighted }) {
  ...
  return (
    <div className={`${base} ${colour}`}>
      <div className={`w-12 h-12 rounded-xl mb-5 grid place-items-center ${highlighted ? 'bg-white/15 text-white' : 'bg-sapphire-light text-sapphire'}`}><Icon size={22} strokeWidth={1.8} /></div>
      <div className={`font-display text-5xl lg:text-6xl font-medium mb-4 ${highlighted ? 'text-white/90' : 'text-ink/70'}`}>{step.num}</div>
      <h3 className={`font-display text-2xl font-semibold mb-3 ${highlighted ? 'text-white' : 'text-ink'}`}>{step.title}</h3>
      <p className={`text-sm leading-relaxed ${highlighted ? 'text-white/85' : 'text-ink-soft'}`}>{step.text}</p>
    </div>
  );
}
