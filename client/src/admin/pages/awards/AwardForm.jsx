...
export default function AwardForm() {
  ...
  const [form, setForm] = useState({ title: '', issuer: '', year: new Date().getFullYear().toString(), description: '', image_url: '', image_public_id: '', sort_order: 0, is_published: true });

  useEffect(() => { if (isEditing) loadItem(); }, [id]);
  async function loadItem() { setLoading(true); const { data } = await supabase.from('awards').select('*').eq('id', id).single(); if (data) setForm(data); setLoading(false); }
  ...
  async function handleSubmit(e) {
    ...
    setSaving(true);
    const { error } = isEditing ? await supabase.from('awards').update(form).eq('id', id) : await supabase.from('awards').insert(form);
    if (error) toast.error(error.message);
    else { toast.success(isEditing ? 'Updated' : 'Created'); await logActivity(isEditing ? 'update' : 'create', 'award', id, form.title); navigate('/admin/awards'); }
    setSaving(false);
  }
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/admin/awards')} className="w-9 h-9 rounded-lg hover:bg-cream dark:hover:bg-ink-line flex items-center justify-center text-muted"><ArrowLeft className="w-5 h-5" /></button>
          <h1 className="text-2xl font-bold text-ink dark:text-white">{isEditing ? 'Edit' : 'Add'} Award</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-ink rounded-xl border border-line dark:border-ink-line p-6 space-y-5">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <FormField label="Issuer" id="issuer"><TextInput id="issuer" value={form.issuer} onChange={(e) => update('issuer', e.target.value)} /></FormField>
        ...
        <ImageUpload label="Certificate Image" value={form.image_url} publicId={form.image_public_id}
          onChange={(url, pid) => setForm((p) => ({ ...p, image_url: url, image_public_id: pid }))} folder="abeywardhane-gems/awards" />
